param(
    [string] $LogPath = '<ARTIFACT_DIR>\patch-bcd-testsigning-only-20260601.log'
)
...
try {
    "=== PIPA BCD TESTSIGNING-ONLY PATCH START $(Get-Date -Format o) ===" | Set-Content -LiteralPath $LogPath -Encoding UTF8
    ...
    Log 'Setting testsigning only; removing nointegritychecks and debug/safemode flags'
    ...
    Log 'DONE'
    "=== PIPA BCD TESTSIGNING-ONLY PATCH END $(Get-Date -Format o) ===" | Add-Content -LiteralPath $LogPath
}
catch {
    try {
