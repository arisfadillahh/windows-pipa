...
LOG=/tmp/pipa-massdisk-0525-timer.log
SECONDS_TO_WAIT="${1:-300}"

exec >>"$LOG" 2>&1
echo "=== pipa mass-disk 0525 timer start ==="
date
...
mkdir -p "$F"
echo 0 > "$F/stall" || true
echo /dev/block/sda > "$F/lun.0/file"
...
echo "exported /dev/block/sda as 0525:a4a5 for ${SECONDS_TO_WAIT}s"
