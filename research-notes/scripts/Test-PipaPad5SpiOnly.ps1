param(
    [string] $Root = 'C:\woa\pad5-spi-only'
)

$ErrorActionPreference = 'Continue'
$LogPath = Join-Path $Root 'test-pad5-spi-only.log'
$ResultPath = Join-Path $Root 'RESULT.txt'
$DonePath = Join-Path $Root 'DONE.txt'
$AttemptPath = Join-Path $Root 'ATTEMPTED.txt'
$InfPath = Join-Path $Root 'driver\MiPad5_spi.inf'
$CatPath = Join-Path $Root 'driver\MiPad5_spi.cat'
$HardwareId = 'ACPI\QCOM050F'
$ExpectedId = 'ACPI\QCOM050F'
$ServiceName = 'qcspi'
...
function Remove-StartupFallback {
    $startupFallback = Join-Path $env:ProgramData 'Microsoft\Windows\Start Menu\Programs\Startup\Codex-Pad5-SPI-Only.cmd'
...
if (Test-Path -LiteralPath $AttemptPath) {
    "SKIPPED: Pad5 SPI test already attempted. Remove ATTEMPTED.txt manually to retry." |
...
$result = @(
    'Pipa isolated Pad5 SPI driver test'
...
Log 'PAD5_SPI_ONLY_TEST_DONE'
Remove-StartupFallback
"=== PIPA PAD5-SPI-ONLY TEST END $(Get-Date -Format o) ===" |
