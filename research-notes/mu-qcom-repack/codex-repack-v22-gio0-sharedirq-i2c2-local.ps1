...
$ScratchRoot = Join-Path $WoaRoot "mu-repack-v22-gio0-sharedirq-i2c2-local"
...
$TouchAsl = Join-Path $ScratchRoot "TouchMinGio0SharedIrqSSDT.asl"
$TouchAml = Join-Path $ScratchRoot "TouchMinGio0SharedIrqSSDT.aml"
...
$TouchText = $TouchText.Replace('DefinitionBlock ("", "SSDT", 2, "PIPA", "TCHMIN", 0x00000001)', 'DefinitionBlock ("", "SSDT", 2, "PIPA", "TCHGIO0", 0x00000001)')
$TouchText = $TouchText.Replace('Name (_CID, "QCOMFFE3")', 'Name (_CID, "QCOM250D")')
$Gio0CrsPattern = '(?s)            Method \(_CRS, 0, NotSerialized\)\r?\n            \{\r?\n                Name \(RBUF, Buffer \(0x56\)\r?\n                \{\r?\n.*?\r?\n                \}\)\r?\n                Return \(RBUF\)\r?\n            \}'
$Gio0CrsReplacement = @'
            Method (_CRS, 0, NotSerialized)
            {
                Name (RBUF, ResourceTemplate ()
...
'@
$TouchTextPatched = [regex]::Replace($TouchText, $Gio0CrsPattern, $Gio0CrsReplacement, 1)
if ($TouchTextPatched -eq $TouchText) {
    throw "Failed to patch GIO0 _CRS raw buffer"
}
$TouchText = $TouchTextPatched
...
$Fd = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v21-gio0-cid250d-i2c2-local.fd"
$FdMap = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v21-gio0-cid250d-i2c2-local.fd.map"
...
$BootImg = Join-Path $ScratchRoot "Mu-pipa-touchmin-v22-gio0-sharedirq-i2c2-local.img"
...
$FinalFd = Join-Path $OutDir "pipa_muold_touchmin_v22-gio0-sharedirq-i2c2-local.fd"
$FinalImg = Join-Path $OutDir "pipa_muold_touchmin_v22-gio0-sharedirq-i2c2-local.img"
