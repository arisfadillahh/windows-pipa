...
$ScratchRoot = Join-Path $WoaRoot "mu-repack-v29-touchprep-nodep-irq637-local"
...
@'
DefinitionBlock ("", "SSDT", 2, "PIPA", "I2C2V29", 0x00000001)
{
...
$TouchText = $TouchText.Replace('DefinitionBlock ("", "SSDT", 2, "PIPA", "TCHMIN", 0x00000001)', 'DefinitionBlock ("", "SSDT", 2, "PIPA", "TCH29TP", 0x00000001)')
...
# --- v29 touch prep (inert until qcspi is installed):
#     1) Drop SPI4 _DEP (PEP0, QGP0). The 2026-06-05 qcspi attempt on the pepdep image
#        bootlooped with BugCheck 0xA0 INTERNAL_POWER_ERROR; qci2c runs fine on I2C2
#        precisely because I2C2 is declared with NO _DEP ("nodep"). Same recipe for SPI4.
#     2) SPI4 IRQ 605 (raw DT hwirq) -> 637 (GSIV = hwirq + 32), same fix as I2C2's 635. ---
$depPattern = '(?s)\r?\n\s*Name \(_DEP, Package \(0x02\)\s*\{\s*PEP0,\s*QGP0\s*\}\)'
if (([regex]::Matches($TouchText, $depPattern)).Count -ne 1) { throw "SPI4 _DEP block not found exactly once" }
$TouchText = [regex]::Replace($TouchText, $depPattern, '')
$irqCount = ([regex]::Matches($TouchText, [regex]::Escape('0x0000025D,'))).Count
if ($irqCount -ne 1) { throw "Expected exactly 1 SPI4 IRQ 0x25D, found $irqCount" }
$TouchText = $TouchText.Replace('0x0000025D,', '0x0000027D,')
...
$Fd = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v29-touchprep-nodep-irq637-local.fd"
$FdMap = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v29-touchprep-nodep-irq637-local.fd.map"
...
$BootImg = Join-Path $ScratchRoot "Mu-pipa-touchmin-v29-touchprep-nodep-irq637-local.img"
...
$FinalFd = Join-Path $OutDir "pipa_muold_touchmin_v29-touchprep-nodep-irq637-local.fd"
$FinalImg = Join-Path $OutDir "pipa_muold_touchmin_v29-touchprep-nodep-irq637-local.img"
