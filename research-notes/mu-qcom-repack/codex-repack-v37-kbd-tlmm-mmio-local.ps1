...
$ScratchRoot = Join-Path $WoaRoot "mu-repack-v37-kbd-tlmm-mmio-local"
...
@'
DefinitionBlock ("", "SSDT", 2, "PIPA", "I2CV37", 0x00000001)
{
...
$TouchText = $TouchText.Replace('DefinitionBlock ("", "SSDT", 2, "PIPA", "TCHMIN", 0x00000001)', 'DefinitionBlock ("", "SSDT", 2, "PIPA", "TCH33KB", 0x00000001)')
...
# --- SEC8: Nanosic 803 keyboard child (\_SB.I2C2.KBD0, _HID NANO0803) so Windows enumerates
#     ACPI\NANO0803 and binds pipakbd.sys. v37 adds TLMM MMIO to KBD0 so pipakbd can
#     directly toggle pipa keyboard power pins when ACPI GPIO/ResourceHub is inert. ---
$KbdAsl = Join-Path $ScratchRoot "NanoKbdSSDT.asl"
$KbdAml = Join-Path $ScratchRoot "NanoKbdSSDT.aml"
@'
DefinitionBlock ("", "SSDT", 2, "PIPA", "NANOQ37", 0x00000005)
{
    ...
    Scope (\_SB.I2C2)
    {
        Device (KBD0)
        {
            ...
            // GPIO pin order: reset, vdd, sleep.
            ...
            OperationRegion (KGIO, GeneralPurposeIo, Zero, 0x03)
            Field (KGIO, ByteAcc, NoLock, Preserve)
            {
                Connection (GPOR),
                KRST, 1,
                KVDD, 1,
            ...
            PowerResource (KPR0, 0x00, 0x0000)
            {
            ...
            {
                Return (ConcatenateResTemplate (I2CR, TLMM))
            }
        }
    }
}
'@ | Set-Content -LiteralPath $KbdAsl -Encoding ASCII
Push-Location $ScratchRoot
try { Run-Tool $Iasl $KbdAsl } finally { Pop-Location }
if (-not (Test-Path -LiteralPath $KbdAml)) { throw "IASL did not produce $KbdAml" }
$KbdRaw = Join-Path $FfsAcpiDir "${AcpiGuid}SEC8.raw"
Run-Tool $GenSec "-s" "EFI_SECTION_RAW" "-o" $KbdRaw $KbdAml
$AcpiInputs += $KbdRaw
...
$Fd = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v37-kbd-tlmm-mmio-local.fd"
$FdMap = Join-Path $ScratchRoot "PIPA_UEFI-touchmin-v37-kbd-tlmm-mmio-local.fd.map"
...
$BootImg = Join-Path $ScratchRoot "Mu-pipa-touchmin-v37-kbd-tlmm-mmio-local.img"
...
$FinalFd = Join-Path $OutDir "pipa_muold_touchmin_v37-kbd-tlmm-mmio-local.fd"
$FinalImg = Join-Path $OutDir "pipa_muold_touchmin_v37-kbd-tlmm-mmio-local.img"
